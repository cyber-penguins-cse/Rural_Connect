        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <Link to="/" className="flex items-center gap-2.5 mb-3">
              <div className="w-8 h-8 bg-[#008000] rounded-xl flex items-center justify-center">
                <Sprout className="w-4 h-4 text-white" />
              </div>
              <span className="font-bold text-lg text-gray-900">Rural<span className="text-[#008000]">Connect</span></span>
            </Link>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[['Browse Products', '/products'], ['Sell Your Crafts', '/register'], ['How It Works', '/']].map(([label, href]) => (
                <li key={label}>
                  <Link to={href} className="text-sm text-gray-500 hover:text-[#008000] transition-colors">{label}</Link>
                </li>
              ))}
            </ul>
          </div>
